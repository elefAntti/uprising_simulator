...
if __name__ == "__main__":
    ...
    cache_path=os.path.join("weights/norm_stats.npz")
    mean,std=load_norm(cache_path=cache_path)
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            for obs_batch, act_batch in val_loader:
                obs_batch, act_batch = normalizer(obs_batch.to(device)), act_batch.to(device)
                pred_actions = model(obs_batch)
                loss = loss_fn(pred_actions, act_batch)
                val_loss += loss.item()
