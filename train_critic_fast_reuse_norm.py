...
def compute_norm_fast(files: List[str], max_files: int, max_batches_per_file: int, batch: int) -> Tuple[torch.Tensor, torch.Tensor, int]:
    ...
    for fi, fp in enumerate(files_scan):
        ...
        for bi in range(n_batches):
            lo = bi * batch; hi = min(n_rows, lo + batch)
            xb = torch.from_numpy(X[lo:hi]).double()
            bs = xb.shape[0]
...
def try_load_norm_stats(out_dir: str):
    npz_path = os.path.join(out_dir, "norm_stats.npz")
    json_path = os.path.join(out_dir, "norm_stats.json")
    if os.path.exists(npz_path):
        z = np.load(npz_path)
        mean = torch.tensor(z["mean"], dtype=torch.float32)
        std  = torch.tensor(z["std"],  dtype=torch.float32)
        print(f"[norm] loaded cached stats from {npz_path} (dim={mean.numel()})")
        return mean, std
    if os.path.exists(json_path):
        ...
        print(f"[norm] loaded cached stats from {json_path} (dim={mean.numel()})")
        return mean, std
    return None, None
...
def train(args):
    ...
    # Reuse existing norm stats if available
    mean_t, std_t = try_load_norm_stats(args.out)
    if mean_t is not None and std_t is not None:
        if mean_t.numel() != d or std_t.numel() != d:
            print(f"[norm] cached stats dim={mean_t.numel()} do not match input_dim={d}; recomputing...")
            mean_t = std_t = None
    if mean_t is None or std_t is None:
        with timer("normalization (compute)"):
            mean_t, std_t, d2 = compute_norm_fast(files, max_files=args.norm_files, max_batches_per_file=args.norm_batches_per_file, batch=args.batch)
        assert d2 == d
        os.makedirs(args.out, exist_ok=True)
        np.savez(os.path.join(args.out, "norm_stats.npz"), mean=mean_t.numpy(), std=std_t.numpy())
        with open(os.path.join(args.out, "norm_stats.json"), "w") as f:
            json.dump({"mean": mean_t.tolist(), "std": std_t.tolist()}, f, indent=2)
    else:
        print("[norm] using cached normalization stats")

    norm = Normalizer(mean_t, std_t)
...
def main():
    ...
    ap.add_argument("--val-split", type=float, default=0.1)  # kept for compatibility
